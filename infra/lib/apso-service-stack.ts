...
export class ApsoServiceStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: InfraProps) {
    ...
    const vpc = getVpc(this, `${id}-vpc`, props.vpcId);
      ...
      this.provisionLambdaService(id, vpc, dbUserSecretArn)
  ...
  private provisionLambdaService(
    ...
    console.log('Template File: ', path.join(__dirname,'../../src/lambda.ts'))

    const container = {
      src: path.join(__dirname, '../..', 'containers', 'lambda', 'Dockerfile'),
      dest: path.join(__dirname, '../..', 'Dockerfile')
    }
